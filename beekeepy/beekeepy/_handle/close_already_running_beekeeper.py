...
def close_already_running_beekeeper(*, working_directory: Path) -> None:
    ...
    try:
        with SynchronousBeekeeperHandle(settings=Settings(working_directory=working_directory), logger=logger):
            """
            While trying to start new beekeeper instance in same directory,
            there will be send notification with information about already existing process.
            """
