...
def test_http(address: HttpUrl) -> bool:
    ...
    try:
        with socket.create_connection(
            (address.address, address.port), timeout=1
        ) as sock:
            sock.sendall(b"GET / HTTP/1.1\r\nHost: localhost\r\n\r\n")
            response = sock.recv(1024)
            return (
                response.startswith(b"HTTP")
                and WEBSERVER_SPECIFIC_RESPONSE not in response
            )
...
def test_https(address: HttpUrl) -> bool:
    ...
    try:
        ...
        with socket.create_connection(
            (address.address, address.port), timeout=1
        ) as sock, context.wrap_socket(sock, server_hostname="localhost") as ssock:
...
def match_ports(ports: list[int], *, address: str = "127.0.0.1") -> PortMatchingResult:
    ...
    for port in ports:
        if categories.http is None and test_http(
            http_result := HttpUrl.factory(port=port, address=address)
        ):
            categories.http = http_result
        elif categories.https is None and test_https(
            http_result := HttpUrl.factory(port=port, address=address)
        ):
            categories.https = http_result
        elif categories.websocket is None and test_websocket(
