...
class _PostRequestManager:
    ...
    def __set_response_or_exception(self, request_id: int, response: dict[str, Any], exception_url: str = "") -> bool:
    ...
    def __handle_no_exception_case(self) -> bool | None:
        self.__validate_response_count(self.__responses)
        for response in self.__responses:
            if not self.__set_response_or_exception(request_id=int(response["id"]), response=response):
                return False
        return None
    ...
    def __handle_exception_and_responses_exists(self, responses: list[JsonT], url: str) -> bool | None:
        for response in responses:
            if not self.__set_response_or_exception(
                request_id=int(response["id"]), response=response, exception_url=url
            ):
                return False
    ...
    def __exit__(
        ...
        if exception is None and len(self.__responses):
            return self.__handle_no_exception_case()
