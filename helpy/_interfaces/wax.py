...
class SecondStepEncryptionCallback(Protocol):
    def __call__(self, from_key: PublicKey, to_key: PublicKey, content: str, nonce: int = 0) -> str:
        ...
...
def encrypt_memo(
    ...
    return encoded_encrypted_memo.decode()


def decrypt_memo(content: str, second_step_callback: SecondStepEncryptionCallback) -> str:
    encrypted_memo = wax.decode_encrypted_memo(encoded_memo=content.encode())
    return second_step_callback(
        from_key=encrypted_memo.main_encryption_key.decode(),
        to_key=encrypted_memo.other_encryption_key.decode(),
        content=encrypted_memo.encrypted_content.decode(),
    )
